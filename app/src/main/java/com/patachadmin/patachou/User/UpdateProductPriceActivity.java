...
public class UpdateProductPriceActivity extends AppCompatActivity {
    EditText product_price;
    private Dialog loadingDialog;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_update_product_price);
    ...
    public void savePrice(View view) {
        if(product_price.getText().toString().trim().isEmpty()){
            ...
            loadingDialog.show();
            FirebaseDatabase.getInstance().getReference("User").child(getUserId(UpdateProductPriceActivity.this)).child(USERID)
                    .child("Products").child(TYPE).child(PRODUCTID)
            .child("ProductPrice").setValue(product_price.getText().toString());
